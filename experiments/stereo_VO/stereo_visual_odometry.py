...
class VisualOdometry():
    def __init__(self, data_dir):
        ...
        self.orb = cv2.ORB_create(nfeatures=2000, scaleFactor=1.2, nlevels=8, edgeThreshold=2, firstLevel=0, WTA_K=2, scoreType=cv2.ORB_HARRIS_SCORE , patchSize=2, fastThreshold=2)
    ...
    def get_tiled_keypoints(self, img, tile_h, tile_w):
        ...
        def get_kps(x, y):
            ...
            if len(keypoints) > 10:
                #pair = sorted(pair, key=lambda x: -x.response)
                sortedpair = sorted(zip(keypoints, descriptors), key=lambda pair: -pair[0].response)
                bestx = sortedpair[:10]
                #bestx = [(x, y) for x, y in bestx if not None in y]
    ...
    def track_keypoints(self, img1, img2, kp1, max_error=8):
        ...
        print(trackpoints1, trackpoints2)
    ...
    '''
    def track_keypoints(self, img1, img2, kp1, kp2_l, ds1_l, ds_2_l, max_error=100):
        """
        Tracks the keypoints between frames
        ...
        print(np.array(trackpoints1).shape)
        return np.array(trackpoints1), np.array(trackpoints2)

    
    def calculate_right_qs(self, q1, q2, disp1, disp2, min_disp=0.0, max_disp=3000.0):
        """
    ...
    def get_pose(self, i):
        ...
        kp1_l, ds1_l = self.get_tiled_keypoints(img1_l, 24, 48) #10, 20
        kp2_l, ds2_l = self.get_tiled_keypoints(img2_l, 24, 48)

        # Track the keypoints
        tp1_l, tp2_l = self.track_keypoints(img1_l, img2_l, kp1_l, kp2_l, ds1_l, ds2_l)
